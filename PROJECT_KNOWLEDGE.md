...
*   **24 Agustus 2026** - *Toggle 2 Mode Transaksi, Direct DM Fix & Penyempurnaan UI/UX* - (1) Menambahkan fitur Toggle Global Mode Transaksi (Mode 1: Direct WhatsApp vs Mode 2: In-App DM Web) di Admin Panel dan tabel `system_settings`. Tombol Minat & Tawaran otomatis beralih fungsi dinamis. (2) Mengembangkan `chat_rooms_type_check` di PostgreSQL untuk mengizinkan tipe `direct` pada fitur Saling Setuju Lanjut DM Pribadi. (3) Memperbaiki styling form Jual Barang di Dark Mode (`floating-input`), safe-area header mobile, dan menyaring penumpukan pemisah sesi kosong di Cari Temen.
