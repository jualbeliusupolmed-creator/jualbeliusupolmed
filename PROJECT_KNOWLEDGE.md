...
*   **29 Agustus 2026 — `formatWa()` menolak SETIAP JID; otak WhatsApp situs diam sejak 26 Agustus**
    ...
    - **Akibat kedua:** pemanggil yang punya cadangan `jid.split("@")[0]` — `convoWa()` di webhook dan `logBotSend()` di `lib/fonnte.js` — menulis digit mentah berformat **62** ke `wa_conversations`, sementara 981 baris lain memakai **08**. Satu orang jadi dua kunci: riwayat chat terbelah di panel admin, penjaga "sapa sekali seumur hidup" tidak menemukan sapaannya sendiri, dan pembeda "echo bot vs balasan manual admin" kehilangan catatan kiriman botnya.
    - **Perbaikannya** memangkas sufiks JID **sebelum** penjaga sintetis berjalan. Perlindungan yang jadi alasan penjaga itu ada tetap utuh — bagian sebelum `@` pada `email_0812345_1234` masih berhuruf, jadi masih ditolak, termasuk kalau ia datang lengkap dengan sufiks JID. Dikunci oleh `__tests__/formatWa.test.js`.
    - **Diverifikasi di produksi setelah deploy:** `/penjual/6282111946196%40s.whatsapp.net` dan `/penjual/082111946196%40c.us` menjawab **200** (sebelumnya 404), sementara `/penjual/email_0812345_1234` tetap **404**.
    - **Pelajarannya:** penjaga yang menolak "apa pun yang berhuruf" harus tahu **bagian mana** dari string yang boleh berhuruf. Dan gejala "bot menyapa lalu diam" tidak membuktikan botnya hidup — sapaan bisa datang dari lapisan yang berbeda dengan yang menjawab.
    - **⚠ Menunggu pemilik — enam baris data yang terlanjur terbelah.** Kodenya sudah berhenti menambah, tapi yang sudah telanjur ada tidak ikut sembuh sendiri: `wa_conversations` id 999/1000/1002/1003 (4 baris, 28 Agustus) plus penjual seed `6283456789003` di `seller_profiles` (ikut `listings` lewat FK `ON UPDATE CASCADE`). Selama belum disatukan, `/penjual/083456789003` yang terdaftar di peta situs tetap 404 dan empat orang itu bisa disapa ulang. Perintahnya ada di `supabase/migration_satukan_kunci_wa.sql` — **penyaring izin menolak UPDATE massal dari sesi ini**, jadi ia harus dijalankan pemilik.
    - **Sekalian:** `/blog` dicabut dari peta situs — satu-satunya dari 99 URL yang menjawab 307 (ke `/mading?tab=blog`). Tulisan blognya sendiri tetap terdaftar.
...
*   **28 Agustus 2026** - *Audit situs produksi: soft-404, menu mati, dan akar repo* — semua temuan dibuktikan dengan mengetuk `https://www.jualbeliusupolmed.web.id`, bukan dengan membaca kode saja.
    ...
    - **Diterapkan 29 Agustus 2026 (versi `20260829021232`).** Dua peringatan WARN Performance Advisor yang tersisa ditutup lewat `supabase/migration_rapikan_indeks_dan_policy.sql`. (1) Indeks kembar `idx_listings_listing_code_unique` dibuang — diperiksa dulu lewat `pg_constraint` bahwa yang menopang UNIQUE constraint adalah `listings_listing_code_unique`, dan sesudahnya dipastikan constraint `u` itu masih berdiri dengan satu indeks pendukung. Yang dihemat bukan disk melainkan tulis: tiap INSERT/UPDATE `listings` tadinya memelihara dua B-tree identik. **Catatan yang sempat menipu:** indeks lepas itu justru punya 12 `idx_scan` sementara indeks constraint baru 2 — perencana memang boleh memilih salah satu dari dua indeks identik, dan angka pemakaian bukan alasan mempertahankan salinan. Kalau perlu dibalik, satu baris: `CREATE UNIQUE INDEX idx_listings_listing_code_unique ON public.listings (listing_code);`. (2) Policy `service_role_full_access_buyer_contacts` dibuat ulang dengan `(select auth.role())` supaya dievaluasi sekali per query, bukan sekali per baris — semantiknya tidak berubah, dan diverifikasi lewat `pg_get_expr`. Sesudahnya Performance Advisor tinggal 22 lint INFO "Unused Index" yang sengaja dibiarkan: hampir semuanya milik tabel fitur berumur seminggu, dan "belum pernah terpakai" di situ bukan bukti indeksnya salah. Security Advisor: tidak ada temuan baru; peringatan `process_teman_swipe` yang dulu tercatat sudah tidak muncul lagi.
...
# 2026-09-16 — Memori percakapan AI WhatsApp
...
*   **19 September 2026 — Standardisasi Aksesibilitas Modal WCAG AA, Focus Trap, & Hierarki Panel Admin**
    - **Aksesibilitas Modal & Hook `useFocusTrap`:** Menambahkan hook terpusat `src/lib/useFocusTrap.js` dengan pengujian unit otomatis (`__tests__/useFocusTrap.test.jsx`, 5 tes lulus). Menangani isolasi tab navigasi, escape-to-close, body scroll locking, dan pemulihan fokus ke trigger asal.
    - **Portalling Menyeluruh:** Seluruh modal dan drawer interaktif (`MatchModal`, `PhotoUploadModal`, `BuatOprecModal`, `OprecDaftarModal`, `OTPModal`, `QRISModal`, `UnduhMenfessModal`) di-portal langsung ke `document.body` untuk mencegah nesting z-index dan stacking context bugs.
    - **Hierarki Menu Admin (`nav.js` & `AdminNav.jsx`):** Mereorganisasi navigasi admin menjadi kategori operasional yang logis: *Ringkasan*, *Perlu Tindakan* (Moderasi, Laporan, Toko, Profil, Transaksi), *Marketplace*, *Pengguna*, *Komunikasi*, *Laporan*, dan *Sistem*. Mengarahkan perhatian staf admin ke item yang butuh aksi cepat.
    - **Performa & Sinkronisasi Favorit:** Mengalihkan revalidasi favorit pengguna di `/favorit` dari query client-side langsung ke endpoint batch `/api/listings/batch` dengan preservasi urutan lokal dan Zero Layout Shift.
    - **Typography & Icon Token:** Membersihkan artefak UTF-8 BOM pada `globals.css` serta mendefinisikan token skala teks padat (`.text-badge`, `.text-meta`, `.text-caption`) dan standardisasi dimensi ikon UI (`h-4 w-4`).
