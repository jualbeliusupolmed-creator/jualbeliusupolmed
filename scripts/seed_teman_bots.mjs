...
// 10 Profil Mahasiswa USU & POLMED dengan Foto Profil Blank (Default Placeholder)
const AUTHENTIC_MEDAN_STUDENTS_BLANK_PP = [
  {
...
async function run() {
  console.log("Updating bot profiles to have blank photo_url...");

  // Hapus bot lama jika ada agar bersih
  await supabase.from("teman_profiles").delete().like("user_id", "bot_%");

  for (const bot of AUTHENTIC_MEDAN_STUDENTS_BLANK_PP) {
    ...
    if (error) {
      ...
      console.log(`✓ [BLANK PP] ${bot.display_name} (${bot.campus} - ${bot.faculty}) [ID: ${data[0]?.id}]`);
    }
  }

  console.log("Update complete! All 10 bots now have clean blank profile pictures.");
}
