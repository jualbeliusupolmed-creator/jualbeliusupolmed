  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true">
      ...
      <div className="relative w-full max-w-sm max-h-[92vh] overflow-y-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden rounded-[28px] bg-white/95 backdrop-blur-xl p-6 shadow-[0_32px_64px_rgba(0,0,0,0.25)] dark:bg-[#1c1c1e]/95 border border-black/[0.05] dark:border-white/[0.08] animate-slide-up">
        ...
        <h3 className="mb-1 text-[18px] font-extrabold tracking-tight text-[#1d1d1f] dark:text-white flex items-center gap-2">
          Masuk / Daftar
        </h3>
        <p className="mb-4 text-[13px] text-[#6e6e73] dark:text-slate-400">
          Masuk instan dengan Google atau gunakan akun WhatsApp &amp; Email.
        </p>

        {/* ── GOOGLE LOGIN PROMINEN DI ATAS ── */}
        <button
          type="button"
          onClick={handleGoogleLogin}
          disabled={googleBusy}
          className="w-full flex items-center justify-center gap-3 rounded-2xl border border-black/[0.08] dark:border-white/[0.12] bg-white dark:bg-[#2c2c2e] py-3 px-4 text-sm font-bold text-gray-800 dark:text-white hover:bg-gray-50 dark:hover:bg-[#3a3a3c] active:scale-[0.98] transition-all shadow-xs disabled:opacity-60"
        >
          {googleBusy ? (
          ...
          <span>{googleBusy ? "Mengarahkan ke Google..." : "Lanjutkan dengan Google"}</span>
        </button>

        {/* Divider */}
        <div className="my-4 relative flex items-center gap-3">
          <div className="flex-1 border-t border-black/[0.06] dark:border-slate-800" />
