  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.error || "Gagal memverifikasi struk. Pastikan struk jelas dan nominal sesuai.");
      }
      ...
      <div className="card w-full max-w-md bg-white p-6 shadow-2xl dark:bg-slate-900/95 dark:border-slate-800 animate-fade-in relative my-8">
        ...
        <div className="text-center mt-2">
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-slate-400">
            Scan QRIS Statis di bawah ini dengan m-Banking / e-Wallet Anda:
          </p>
          ...
          {fee && (
            <div className="mt-3 p-3 rounded-xl bg-gray-50 dark:bg-slate-950 border border-gray-150/40 dark:border-slate-850">
              ...
              <p className="text-[10px] text-red-500 mt-1 font-bold">PENTING: Jangan bulatkan nominal! Harus persis.</p>
            </div>
          )}

          <div className="mt-5 border-t border-gray-100 dark:border-slate-800 pt-4 text-left">
            <label className="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">
              📸 Upload Bukti Transfer
            </label>
            <p className="text-xs text-gray-500 dark:text-slate-400 mb-3">
              Setelah transfer sukses, screenshot struk dan upload di sini. Robot AI kami akan memverifikasi dalam hitungan detik.
            </p>
          ...
          <div className="mt-5 space-y-2.5">
            <button
              ...
              className="btn-primary w-full text-center py-3 shadow-md transition active:scale-95 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
            ...
            {!loading && !successMsg && (
              <button
                type="button"
                onClick={onClose}
                className="w-full text-center py-2 text-sm text-gray-500 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200"
              >
                Batal
              </button>
            )}
