...
export default function ProductCard({ listing, tanpaPenjual = false }) {
  ...
  return (
    <div className="card group relative overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:border-primary/30 hover:shadow-glow dark:border-slate-800/60 dark:bg-slate-900/40 dark:hover:border-primary/40 dark:hover:bg-slate-900/60">
      ...
      <Link href={`/produk/${buildSlug(listing.title, listing.id)}`} className="block">
        <div className="relative aspect-square overflow-hidden bg-gray-100 dark:bg-slate-950">
          ...
          <div className="absolute left-2 top-2 flex flex-col gap-1.5 items-start">
            {isRental && !sold && (
              <span className="rounded-md bg-teal-600/95 backdrop-blur-sm px-2 py-0.5 text-[11px] font-bold text-white shadow-soft">
                🔑 SEWA
              </span>
            )}
            {!isRental && isSponsored && !sold && (
              <span className="rounded-md bg-indigo-600/95 backdrop-blur-sm px-2 py-0.5 text-[11px] font-bold text-white shadow-soft">
                📢 Sponsor
              </span>
            )}
            {!isRental && !isSponsored && listing.featured && (
              <span className="rounded-md bg-white/95 backdrop-blur-sm px-2 py-0.5 text-[11px] font-semibold text-gray-900 shadow-soft dark:bg-slate-900/95 dark:text-slate-100">
                Unggulan
              </span>
            )}
            {!isRental && !isSponsored && !listing.featured && isNew && !sold && (
              <span className="rounded-md bg-emerald-500/95 backdrop-blur-sm px-2 py-0.5 text-[11px] font-bold text-white shadow-soft flex items-center gap-1">
                <Icon.Star className="h-3 w-3" /> BARU
              </span>
            )}
            {!isRental && !isSponsored && !listing.featured && isConditionNew && !isNew && !sold && (
              <span className="rounded-md bg-sky-500/95 backdrop-blur-sm px-2 py-0.5 text-[11px] font-bold text-white shadow-soft">
                Baru
              </span>
            )}
          </div>
          ...
          {isLowStock && !sold && (
            <span className="absolute bottom-2 left-2 rounded-md bg-amber-500/95 backdrop-blur-sm px-2 py-0.5 text-[10px] font-bold text-white shadow-soft">
              Stok Tipis
            </span>
          )}
          {sold && (
            <span className="absolute inset-0 grid place-items-center bg-white/60 dark:bg-black/60 backdrop-blur-[2px]">
              <span className="rounded-full bg-gray-900 px-4 py-1.5 text-xs font-bold tracking-wide text-white dark:bg-white dark:text-gray-900 shadow-soft">
                TERJUAL
              </span>
            </span>
          )}
        ...
        <div className="p-3.5">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:text-slate-500 bg-gray-100/50 dark:bg-slate-800/50 px-1.5 py-0.5 rounded-sm">
              {listing.category}
            </span>
            <span className="text-[10px] px-1.5 py-0.5 rounded-sm bg-primary/5 text-primary dark:bg-emerald-500/10 dark:text-emerald-400 font-semibold max-w-[140px] truncate flex items-center gap-1">
          ...
          <h3 className="mt-2 line-clamp-2 text-sm font-semibold leading-snug text-gray-900 dark:text-slate-200 group-hover:text-primary dark:group-hover:text-emerald-400 transition-colors">
            {listing.title}
          </h3>
          <p className="mt-2 text-[16px] font-extrabold tracking-tight text-gray-900 dark:text-white">
            {listing.type === "jasa" && (
              <span className="text-xs font-medium text-gray-500 dark:text-slate-400 mr-1">Mulai</span>
            )}
            {rupiah(listing.price)}
            {listing.type === "sewa" && listing.rental_period && (
              <span className="text-xs font-semibold text-teal-600 dark:text-teal-400 ml-1">/{listing.rental_period}</span>
            )}
          </p>
          {isDistributor && distributorFee > 0 && (
            <p className="text-[10px] text-orange-500 dark:text-orange-400 font-semibold mt-1">
              Fee distributor: {rupiah(distributorFee)}
            </p>
          )}
          <div className="mt-2.5 pt-2.5 border-t border-gray-100 dark:border-slate-800/60 flex items-center justify-between text-[11px] text-gray-500 dark:text-slate-400">
            ...
            <div className="shrink-0 flex items-center gap-1.5">
              {isNego && (
                <span className="rounded bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-widest">
                  Nego
                </span>
              )}
