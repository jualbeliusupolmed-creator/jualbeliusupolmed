      ...
      {rooms.length ? (
        <div className="space-y-4">
          {rooms.map((room) => {
            ...
            const wa1 = room.user1_id ? waMap[room.user1_id] : null;
            const wa2 = room.user2_id ? waMap[room.user2_id] : null;
                  ...
                  <div className="flex flex-col gap-1.5">
                    <div className="flex flex-wrap items-center gap-2 text-xs">
                      ...
                      <div className="flex items-center gap-1 bg-white dark:bg-slate-800 px-2.5 py-1 rounded-xl border border-slate-200/70 dark:border-slate-700">
                        ...
                        <span className="text-[10px] text-slate-400">({room.user1_faculty || "Umum"})</span>
                        {wa1 && (
                          <>
                            <Link
                      ...
                      <div className="flex items-center gap-1 bg-white dark:bg-slate-800 px-2.5 py-1 rounded-xl border border-slate-200/70 dark:border-slate-700">
                        ...
                        {room.user2_alias && <span className="text-[10px] text-slate-400">({room.user2_faculty || "Umum"})</span>}
                        {wa2 && (
                          <>
                            <Link
                ...
                <div className="max-h-80 space-y-2 overflow-y-auto p-4 bg-slate-50/30 dark:bg-slate-900/30">
                  {roomMessages.length ? (
                    roomMessages.map((message) => {
                      ...
                      const senderWa = message.sender_id ? waMap[message.sender_id] : null;
                        ...
                        >
                          <div className="mb-1 flex items-center justify-between gap-3 text-[11px]">
                            <div className="flex items-center gap-1.5 font-black text-slate-800 dark:text-slate-200">
                              <span className={isUser1 ? "text-primary dark:text-emerald-400" : "text-emerald-700 dark:text-emerald-400"}>
        ...
        <div className="rounded-2xl border border-dashed border-slate-300 p-10 text-center text-xs text-slate-500 dark:border-slate-700">
          {searchQuery ? `Tidak ada percakapan yang cocok dengan pencarian "${searchQuery}".` : "Belum ada percakapan Cari Teman tercatat."}
        </div>
      )}
    </div>
  );
