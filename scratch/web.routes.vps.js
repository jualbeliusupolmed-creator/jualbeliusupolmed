...
module.exports = function pasangRuteHalaman(app, ctx) {
    ...
    // Roadmap produk adalah catatan kerja panel, bukan halaman publik.
    app.get('/future', requireAuthPage, (req, res) => {
        res.sendFile(path.join(AKAR, 'halaman', 'future.html'));
    });

    // ── Halaman Infrastruktur & Kredensial (butuh sandi) ────────────────────────
    app.get(['/infrastruktur', '/kredensial'], requireAuthPage, (req, res) => {
        res.sendFile(path.join(AKAR, 'halaman', 'infrastruktur.html'));
    });
